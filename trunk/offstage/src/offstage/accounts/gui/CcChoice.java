...
//	TypedWidgetMap map = new TypedWidgetMap();
	...
	tf = new JTypedTextField();
		tf.setJType(String.class, JTypedTextField.newFormatterFactory(new CCFormatter()));
		addWidget("ccnumber", tf);
	tf = new JTypedTextField();
		tf.setJType(String.class, JTypedTextField.newFormatterFactory(new ExpDateFormatter()));
		addWidget("expdate", tf);
	tf = new JTypedTextField();
		tf.setJType(String.class, JTypedTextField.newFormatterFactory(new DigitsFormatter(3)));
		addWidget("seccode", tf);
	tf = new JTypedTextField();
		tf.setJType(String.class, JTypedTextField.newFormatterFactory(new DigitsFormatter(5)));
		addWidget("zip", tf);
